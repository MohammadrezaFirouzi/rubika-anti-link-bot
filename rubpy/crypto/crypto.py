...
class Crypto(object):
    ...
    def decode_auth(auth: str) -> str:
        ...
        translation_table_lower = str.maketrans(ascii_lowercase, ''.join(
            [chr(((32 - (ord(c) - 97)) % 26) + 97) for c in ascii_lowercase]))
        translation_table_upper = str.maketrans(ascii_uppercase, ''.join(
            [chr(((29 - (ord(c) - 65)) % 26) + 65) for c in ascii_uppercase]))
    ...
    def decrypt(cls, data, key):
        ...
        result, _ = decoder.raw_decode(
            cipher.decrypt(decoded_data).decode('utf-8'))
        return result
    ...
    def create_keys() -> tuple:
        ...
        public_key = Crypto.decode_auth(base64.b64encode(
            keys.publickey().export_key()).decode('utf-8'))
