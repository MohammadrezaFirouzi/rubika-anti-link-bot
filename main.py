...
def check_admins(group_guid ,member_guid : str):
    admins = [i["member_guid"] for i in client.get_group_admin_members(group_guid)['in_chat_members']]
    ...
    def updates(message: Message):
        ...
        if Advertise.is_link(text):
            ...
            if link == True and not check_admins(message.object_guid,message.author_guid):
                ...
                message.reply("کاربر به دلیل ارسال لینک تبلیغاتی از گروه اخراج شد 🎊")
            ...
            if forward == True and not check_admins(message.object_guid,message.author_guid):
                client.ban_group_member(message.object_guid, message.author_guid)
                client.delete_messages(message.object_guid,[message.message_id])
                message.reply("کاربر به دلیل ارسال پیام فرواردی از گروه اخراج شد 🎊")
            ...
            if RubinoPost == True and not check_admins(message.object_guid,message.author_guid):
                client.ban_group_member(message.object_guid, message.author_guid)
                client.delete_messages(message.object_guid,[message.message_id])
                message.reply("کاربر به دلیل ارسال پست روبینو از گروه اخراج شد 🎊")
            ...
            if check_admins(message.object_guid,message.author_guid):
                if forward != False:
                    forward = False
                    message.reply("اوکیه قفل فوروارد خاموش شد\nدیگه عمه کسی در خطر نیست راحت باشین🗿")
                else:
            ...
            if check_admins(message.object_guid,message.author_guid):
                if forward != True:
                    forward = True
                    message.reply("اوکیه فوروارد قفل شد\nعمه هرکی فروارد بزنه🗿")
                else:
            ...
            if check_admins(message.object_guid,message.author_guid):
                if link != False:
                    link = False
                    message.reply("اوکیه قفل لینک خاموش شد\nدیگه عمه کسی در خطر نیست راحت باشین🗿")
                else:
            ...
            if check_admins(message.object_guid,message.author_guid):
                if RubinoPost != True:
                    RubinoPost = True
                    message.reply("اوکیه پست روبینو قفل شد\nعمه هرکی پست روبینو بفرسته🗿")
                else:
            ...
            if check_admins(message.object_guid,message.author_guid):
                if RubinoPost != False:
                    RubinoPost = False
                    message.reply("اوکیه قفل پست روبینو خاموش شد\nدیگه عمه کسی در خطر نیست راحت باشین🗿")
                else:
            ...
            if check_admins(message.object_guid,message.author_guid):
                if StoryRubino != True:
                    StoryRubino = True
                    message.reply("اوکیه استوری روبینو قفل شد\nعمه هرکی استوری روبینو بفرسته🗿")
                else:
            ...
            if check_admins(message.object_guid,message.author_guid):
                if StoryRubino != False:
                    StoryRubino = False
                    message.reply("اوکیه قفل استوری روبینو خاموش شد\nدیگه عمه کسی در خطر نیست راحت باشین🗿")
                else:
